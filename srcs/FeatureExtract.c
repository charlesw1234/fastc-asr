...
void FeatureExtract_Destroy(FeatureExtract_t *self) {
    ...
    FeatureQueue_Destroy(&self->fqueue);
    SpeechWrap_Destroy(&self->speech);
}

static void melspect(FeatureExtract_t *self, const value_t *din, value_t *dout);
void FeatureExtract_Insert(FeatureExtract_t *self, const value_t *din, size_t len, flag_t flag) {
    const value_t *window = self->model->window;
    size_t window_size = self->model->window_size;
    size_t window_shift = 160; // = FRAME_SIZE * 2 ?
    size_t window_start, window_at, max_window_start;
    value_t temp_mean, prev_value, cur_value, temp_feature[FRAME_SIZE];

    /* if (self->mode != 3) {
	window = (const value_t *)window_hex; window_size = window_hex_size;
    } else {
	window = (const value_t *)window_hamm_hex; window_size = window_hamm_hex_size;
	} */
    SpeechWrap_SetTail(&self->speech, din, len);

    assert(SpeechWrap_Size(&self->speech) >= window_size);
    max_window_start = SpeechWrap_Size(&self->speech) - window_size;
    //if (self->mode == 0 || self->mode == 2 || self->mode == 3)
    if (self->model->fqueue_reinit)
	// FeatureQueue_ReInit(&self->fqueue, (max_window_start + window_shift - 1) / window_shift);
	FeatureQueue_ReInit(&self->fqueue, max_window_start / window_shift + 1);

    for (window_start = 0; window_start <= max_window_start; window_start += window_shift) {
	temp_mean = 0;
	for (window_at = 0; window_at < window_size; ++window_at)
	    temp_mean += SpeechWrap_Get(&self->speech, window_start + window_at);
	temp_mean = temp_mean / window_size;
	prev_value = SpeechWrap_Get(&self->speech, window_start) - temp_mean;
	for (window_at = 0; window_at < window_size; ++window_at) {
	    cur_value = SpeechWrap_Get(&self->speech, window_start + window_at) - temp_mean;
	    self->fft_input[window_at] = window[window_at] * (cur_value - 0.97 * prev_value);
	...
	else FeatureQueue_PushFrame(&self->fqueue, temp_feature);
    }
    SpeechWrap_Consume(&self->speech, window_start);
}

#include "predefines/melcoe.c"
static void melspect(FeatureExtract_t *self, const value_t *din, value_t *dout) {
    value_t real, imag, fftmag[256];
    const value_t *melcoe = (const value_t *)melcoe_hex;

    assert(sizeof(value_t) == sizeof(melcoe_hex[0]));
    for (size_t at = 0; at < 256; ++at) {
	real = din[at + at]; imag = din[at + at + 1];
	fftmag[at] = real * real + imag * imag;
    }
    ...
    Model_GlobalCMVN(self->model, dout);
#if 0
    // global cmvn.
    if (self->mode < 2) {
	const value_t *std, *mean;
	if (self->mode == 0) {
	    std = (const value_t *)global_cmvn_std_hex;
	    assert(global_cmvn_std_hex_size == FRAME_SIZE);
	    mean = (const value_t *)global_cmvn_mean_hex;
	    assert(global_cmvn_mean_hex_size == FRAME_SIZE);
	} else { // self->mode == 1.
	    std = (const value_t *)global_cmvn_std_online_hex;
	    assert(global_cmvn_std_online_hex_size == FRAME_SIZE);
	    mean = (const value_t *)global_cmvn_mean_online_hex;
	    assert(global_cmvn_mean_online_hex_size == FRAME_SIZE);
	}
	for (size_t frame_at = 0; frame_at < FRAME_SIZE; ++frame_at)
	    dout[frame_at] = (log(dout[frame_at] < 1e-7 ? 1e-7 : dout[frame_at])
			      - mean[frame_at]) / std[frame_at];
    } else {
